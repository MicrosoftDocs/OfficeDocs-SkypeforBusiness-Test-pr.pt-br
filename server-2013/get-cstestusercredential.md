...
ms:mtpsurl: https://technet.microsoft.com/pt-br/library/JJ204759(v=OCS.15)
ms:contentKeyID: 49306227
ms.date: 05/19/2016
...
The commands shown in Example 2 return a list of all the users who have been configured as watcher node test users. To do this, the first command in the example sets the value of the Interface da linha de comando do Windows PowerShell $ErrorActionPreference variable to "SilentlyContinue"; this suppresses the display of any error messages that would otherwise appear if the **Get-CsTestUserCredential** cmdlet tries to return test user information for a user who has not been configured as a watcher node test user.
...
**Painel de Controle do Lync Server:** The functions carried out by the **Get-CsTestUserCredential** cmdlet are not available in the Painel de Controle do Lync Server.
