...
ms:mtpsurl: https://technet.microsoft.com/pt-br/library/Gg398522(v=OCS.15)
ms:contentKeyID: 49307051
ms.date: 05/19/2016
...
Creates a new Lync Server announcement. Announcements are played when users dial a valid but unassigned phone number. An announcement can be a message (such as "This number is temporarily out of service") or a busy signal. Este cmdlet foi introduzido no Lync Server 2010.
...
Example 2 is similar to Example 1 in that we begin by entering the required parameters, Identity and Name. In this example, however, instead of a TTS prompt, we want the announcement to play an audio file. To do that we include the AudioFilePrompt parameter and pass it a string containing the name of the audio file (WelcomeMessage.wav). This file must be in the Repositório de Arquivos in order for this announcement to play. You can add audio files to the Repositório de Arquivos by calling the **Import-CsAnnouncementFile** cmdlet.
...
<td><p>Optional</p></td>
<td><p>System.String</p></td>
<td><p>The name of the audio file to be played for the announcement. Audio files are stored in the Repositório de Arquivos. To save an audio file to the Repositório de Arquivos, use the <strong>Import-CsAnnouncementFile</strong> cmdlet.</p>
